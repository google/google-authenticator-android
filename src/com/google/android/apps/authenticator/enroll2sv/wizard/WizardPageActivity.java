...
/**
 * Base class for a page of the enrollment flow wizard.
 *
 * <p>
 ...
 * <li>Automatically passes {@link WizardState} through the pages if they are launched with
 * {@link #startPageActivity(Class)},</li>
 * </ul>
 *
 * @author klyubin@google.com (Alex Klyubin)
 */
public class WizardPageActivity extends TestableActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (savedInstanceState == null) {
      WizardState wizardState = getWizardStateFromIntent(getIntent());
      if (wizardState == null) {
        throw new IllegalArgumentException("Wizard state not provided");
      }
      mWizardState = wizardState;
    } else {
      mWizardState = (WizardState) savedInstanceState.getSerializable(KEY_WIZARD_STATE);
    }

    setContentView(R.layout.enroll2sv_wizard_page);
   ...
   * state of the wizard.
   */
  protected void startPageActivity(Class<? extends WizardPageActivity> activityClass) {
  ...
  protected void startPageActivityForResult(
      Class<? extends WizardPageActivity> activityClass, int requestCode) {
    Intent intent = new Intent(this, activityClass);
    intent.putExtra(KEY_WIZARD_STATE, getWizardState());
